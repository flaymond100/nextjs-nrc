...
/**
 * Uploads an image file to Supabase Storage for series images
 * @param file - The image file to upload
 * @param userId - The user's UUID
 * @param bucketName - The storage bucket name (default: "series-images", falls back to "avatars")
 * @returns The public URL of the uploaded file, or null if upload fails
 */
export async function uploadSeriesImage(
  file: File,
  userId: string,
  bucketName: string = "series-images"
): Promise<string | null> {
  try {
    // Validate file size (max 5MB for series images)
    if (file.size > 5 * 1024 * 1024) {
      throw new Error("Image size must be less than 5MB");
    }
    ...
    const fileExt = file.name.split(".").pop();
    const fileName = `series-${userId}-${Date.now()}.${fileExt}`;
    const filePath = fileName;

...
import { StoreManagement, StoreProduct } from "@/utils/types";
...
export default function StoreAdminPage() {
  ...
  const fetchStoreId = async () => {
    try {
      ...
      if (queryError) {
        console.error("Error fetching store id:", queryError);
        return;
      }
  ...
  const handleSaveEdit = async (product: StoreProduct) => {
    if (!editForm) return;
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Store Admin</h1>
          </div>
          <button
            onClick={() => {

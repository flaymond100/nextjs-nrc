...
import { CartItem, Gender, Size, StoreProduct } from "@/utils/types";
import {
  clearCart,
  getCart,
  removeFromCart,
...
type StoreCartItem = CartItem & { category: string };
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      const catalogByVariantId = new Map(
        ((catalogRows || []) as StoreProduct[]).map((row) => [
      ...
      const linkedStoreRows = cartItems.map((item) => {
        ...
        if (!product || !orderItemId) {
          throw new Error(
            `Failed to link ${item.productName} into store rows.`
          );
        }
      ...
      navigate(`/dashboard/store/confirmation?orderId=${order.id}`);
    } catch (err: any) {

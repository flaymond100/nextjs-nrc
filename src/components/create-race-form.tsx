    ...
    onSubmit: async (values) => {
      ...
      try {
        const raceData = {
          ...
          participants: values.participants.length > 0 ? values.participants : null,
        };
          ...
          <div className="mb-6">
            ...
            {loadingRiders ? (
              ...
              <div className="max-h-48 overflow-y-auto border border-gray-300 rounded p-3">
                {riders.map((rider) => {
                  const riderName = `${rider.firstName || ""} ${rider.lastName || ""}`.trim() || rider.uuid;
                  const isSelected = formik.values.participants.includes(rider.uuid);
                  return (
                    <label
                      key={rider.uuid}
